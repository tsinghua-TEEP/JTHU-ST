...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title{
\vspace{1cm} \erhao{\bf 清华大学学报（自然科学版）中文题目\\
\LaTeX 模板（非官方）} \vspace{-0.5cm} \\
\wuhao （名词性短语，少于20字，尽量不使用外文缩写词）\vspace{-0.2cm}
}
...
\author{ \sihao\kai 
  ...
  巴\,九 \makebox{$^{\text{1,2}}$}，
  作者三 \makebox{$^{\text{2}}$}，
  作者四 \makebox{$^{\text{2}}$} \\[0.1cm]
\liuhao (
  1.~~清华大学~~自动化系，北京~~100084； \\
\liuhao 2.~~清华大学~~机械工程系，北京~~100084) 
}
...
% 奇数页页眉
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\fancyhead[CO]{{\footnotesize 伍某某等: 清华学报\LaTeX 模板(非官方)}}            %请在这里写出第一作者以及论文题目
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%      中文摘要
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\CKeyword{关键词1；关键词2；关键词3；关键词4 （3～8个，关键词是名词，不使用缩写词，可在网上查询所选关键词是否被认可）}
\CLCNo{TP393.1}
\Dcode{A}
...
中文摘要（摘要应重点包括4个要素，即研究目的、方法、结果和结论。不少于200汉字。不得简单重复题名、引言、结论中已有的信息；不宜有大量关于研究背景的描述，应避免出现主观性极强的描述；不用非公知公用的符号和术语，不能引用参考文献；缩略语、略称、代号在首次出现时必须加以说明；不出现图、表、公式）
\end{CAbstractJBUAA}
%%%%%%%%首页角注
...
\parbox{\textwidth}{%
{\hei\makebox[\widthof{\makebox{*}收}][r]{收}稿日期: }2022-04-25\\% 录用日期： 2015-**-**； 网络出版时间：（此行信息已填） \\%
% \hei\makebox[\widthof{\makebox{*}收}][r]{网}络出版地址: （已填）\\
{\hei\makebox[\widthof{\makebox{*}收}][r]{基}金项目：}国家自然科学基金面上项目(234567,567456)；国家自然科学基金重点项目（985365）（项目编号前不带No.；没有基金项目时可将“基金项目”标题删去）\\
{\hei\makebox[\widthof{\makebox{*}收}][r]{\makebox{*}作}者简介：}伍六七（1998－），男，博士研究生。\\
{\hei\makebox[\widthof{\makebox{*}收}][r]{\makebox{*}通}信作者：}巴九，副教授，E-mail: baj@tsinghua.edu.cn\\ 
（通信作者与第一作者相同时，在作者简介后加上E-mail即可，删去“通信作者”标题）\\
